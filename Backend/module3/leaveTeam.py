...
def lambda_handler(event, context):
    ...
    r = send_emails(team_record, member_email)

    # Return a success response
    return {
        'statusCode': 200,
        'body': 'Left team successfully.'
    }
    
def send_emails(team_record, member_email):
    email_body = f"{member_email} has left team {team_record.get('team_name')}"
    email_subject = f"Member left from {team_record.get('team_name')}"
